...
module Puppet::Etc
  class << self
    ...
    # @api private
    # Defines Puppet::Etc::Passwd struct class. Contains all of the
    # original member fields of Etc::Passwd, and additional "canonical_"
    # versions of these fields as well. API compatible with Etc::Passwd.
    Puppet::Etc::Passwd = Struct.new(*Etc::Passwd.members, *Etc::Passwd.members.map { |member| "canonical_#{member}".to_sym })
    ...
    # fields as well. API compatible with Etc::Group.
    Puppet::Etc::Group = Struct.new(*Etc::Group.members, *Etc::Group.members.map { |member| "canonical_#{member}".to_sym })
    ...
    def override_field_values_to_utf8(struct)
      ...
      new_struct = struct.is_a?(Etc::Passwd) ? Puppet::Etc::Passwd.new : Puppet::Etc::Group.new
